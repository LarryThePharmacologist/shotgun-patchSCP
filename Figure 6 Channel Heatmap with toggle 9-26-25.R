# Ion-channel heatmap (IUPHAR + optional SynGO rescue)
...
#   - SynGO-Derived_Ion_Channel_Candidates__HGNC_Named_.csv   (optional rescue list)
...
rescue_path <- "SynGO-Derived_Ion_Channel_Candidates__HGNC_Named_.csv"  # optional
...
# ------------------------------
# IUPHAR: derive ion-channel gene list (downloaded March 2025)
# ------------------------------
...
# keep only ion-channel rows by IUPHARDB Type, if the column exists; otherwise skip this filter
ion_types <- c("LGIC","VGIC","OTHER_IC")
if ("Type" %in% names(iuphar)) {
  iuphar <- dplyr::filter(iuphar, .data$Type %in% ion_types)
}
...
# ------------------------------
# Optional "rescue": SynGO ion channel annotations
# ------------------------------
if (file.exists(rescue_path)) {
  ...
  ion_targets <- bind_rows(ion_targets, anti_join(rescue, ion_targets, by = "hgnc")) %>%
    distinct(hgnc, .keep_all = TRUE)
}

# ------------------------------
# Keep only ion-channel rows from the DIA-NN matrix
# ------------------------------
bin_ic <- bin_mat %>% filter(HGNC_Genes %in% ion_targets$hgnc)

# Annotate family/type for rows
ann_rows <- ion_targets %>%
  transmute(HGNC_Genes = hgnc, Family = family, Type = Type)

bin_ic <- bin_mat %>%
  filter(HGNC_Genes %in% ion_targets$hgnc) %>%
  left_join(ann_rows, by = "HGNC_Genes")


# ---- Toggle: keep duplicates (per protein group) vs collapse to gene-level
# TRUE produces GRIA3 & GRIA3.1; FALSE produces one row per gene
keep_duplicates_as_unique <- FALSE  
if (keep_duplicates_as_unique) {
  # Keep multiple rows per gene (distinguished by PG_ID), add display names like GRIA3, GRIA3.1
  # Ensure we don't keep *literal* duplicate rows (same gene & same PG_ID)
  bin_ic <- bin_ic %>% distinct(HGNC_Genes, PG_ID, .keep_all = TRUE)
...
write_csv(
  ...
  file.path(out_dir, paste0("Ion_channel_detection_matrix_v2", timestamp, ".csv"))
)
# ------------------------------
# Heatmap
# ------------------------------
...
png(file.path(out_dir, paste0("heatmap_ion_channels_v2", timestamp, ".png")),
